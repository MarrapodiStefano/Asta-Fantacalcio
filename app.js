...
function counts(t){let c={P:0,D:0,C:0,A:0};t.players.forEach(p=>c[p.role]++);return `${c.P}P · ${c.D}D · ${c.C}C · ${c.A}A`}
function budget(t){return current.initialCredits-t.spent}
function teamRow(t){return `<div class="teamrow"><div><div class="teamname">${esc(t.name)}</div><div class="counts">${counts(t)} · ${t.players.length} giocatori</div></div><div class="budget">${budget(t)} cr</div></div>`}
